...
function App() {
  ...
  const onGoNextBuy = (stepBuy) =>  {
    ...
    setTimeout(() => {
      switch(stepBuy){
        case "getSolution":
          onBridgeTrenitaliaResponse({endpoint : "getSolutions", result: getSolutionsResponse})
          break;
        case "getInfo":
          onBridgeTrenitaliaResponse({endpoint : "getSolutionInfo", result: getSolutionInfoResponse})
          break;
      }
      setIsLoading(false)
    },TIMEOUT_API_REQUEST)
    ...
    switch(endpoint){
      case "getSolutions": 
        ...
        if(result?.data?.itineraries.length > 0){
          ...
          alert("No solutions founded")
        break;
      case "getSolutionInfo":{
        dispatch({type:"SET_SOLUTION_DETAILS",payload: {result}})
        incrementStep()
        break;
      }
      default:{
    ...
    <ThemeProvider theme={theme}>
      ...
      { <Suspense fallback={() => (<Loader/>)}>
        ...
        {state.step === 1 && state.serviceSelected === "BUY_TICKET" && (
          <BuyTicket searchingTicket={searchingTicket} onGoNextBuy={onGoNextBuy} isKeyboardOpened={isKeyboardOpened} keyboardOpened={state.keyboardOpened} />
        )}

        {state.step === 2 && state.serviceSelected === "BUY_TICKET" && (
          <FindSolution searchingTicket={state.searchingTicket} solutionRecap={solutionRecap} solutions={state.solutions} onGoNextBuy={onGoNextBuy} />
        )}

        {state.step === 3 && state.serviceSelected === "BUY_TICKET" && (
          <ChooseSolution solutionDetails={state.solutionDetails} solutionRecap={state.solutionRecap} />
        )}
      </Suspense> }
    </div>
    </ThemeProvider>
